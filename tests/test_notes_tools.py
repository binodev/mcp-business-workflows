...
class TestCreateTask:
    def test_creates_note_with_correct_fields(self, service: NotesService) -> None:
        out = service.create_task(CreateTaskInput(title="Deploy v2", content="Deploy notes", tags=[NoteTag.ops]))
    ...
    def test_incident_tag_requires_human_review(self, service: NotesService) -> None:
        out = service.create_task(CreateTaskInput(title="P0", content="Critical", tags=[NoteTag.incident]))
    ...
    def test_non_incident_does_not_require_review(self, service: NotesService) -> None:
        out = service.create_task(CreateTaskInput(title="Routine", content="Routine task", tags=[NoteTag.ops]))
...
class TestSearchNotes:
    ...
    def test_filters_by_tag(self, service: NotesService) -> None:
        service.create_task(CreateTaskInput(title="Incident note", content="Body", tags=[NoteTag.incident]))
    ...
    def test_incident_results_require_human_review(self, service: NotesService) -> None:
        service.create_task(CreateTaskInput(title="P0 incident", content="Critical", tags=[NoteTag.incident]))
